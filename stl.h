...
class Double_list {
 ...
 public:
  class iterator {
   ...
   public:
    ...
    iterator &operator--() {
      if (the_node == 0) throw "decremented an empty iterator";
 ...
 private:
  ...
  iterator head_iterator;  // итератор, который всегда указывает на начало
                           // списка
  iterator tail_iterator;  // итератор, который всегда указывает на элемент,
